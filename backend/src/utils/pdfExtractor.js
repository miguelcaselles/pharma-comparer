/**
 * PDF Processing Service for Clinical Trial Data Extraction
 * Uses Gemini API with PDF support to extract structured data
 */
...
/**
 * Extract clinical trial data from PDF file(s) using Gemini
 * @param {Array<string>} pdfPaths - Array of paths to PDF files (main + appendices)
 * @param {string} trialIdentifier - Trial identifier (A or B)
 * @returns {Promise<Object>} - Extracted trial data with proposal
 */
export async function extractFromPDFs(pdfPaths, trialIdentifier = 'A') {
  try {
    ...
    console.log(`📄 Uploading ${pdfPaths.length} PDF file(s) to Gemini...`);

    // Upload all PDF files to Gemini
    const uploadedFiles = [];
    for (const pdfPath of pdfPaths) {
      try {
        ...
        console.log(`✅ Uploaded: ${uploadResponse.file.displayName} (${uploadResponse.file.uri})`);
        uploadedFiles.push(uploadResponse.file);
      } catch (uploadError) {
        console.error(`❌ Failed to upload ${pdfPath}:`, uploadError.message);
        throw new Error(`Failed to upload PDF: ${uploadError.message}`);
      }
    }

    // Wait for files to be processed by Gemini
    ...
    const prompt = `You are a medical data extraction specialist. Analyze the provided clinical trial PDF document(s) and extract structured data.

${uploadedFiles.length > 1 ? 'IMPORTANT: You have been provided with multiple PDF files (main paper + appendices). Please analyze ALL files to extract complete data.' : ''}

REQUIRED DATA SCHEMA:
${JSON.stringify(TRIAL_DATA_SCHEMA, null, 2)}

EXTRACTION INSTRUCTIONS:
1. Carefully read through the entire document(s) to find all relevant data
2. Extract all available data that matches the schema
3. For missing data, use null (not 0 or empty string)
4. Be extremely precise with numerical values:
   - Hazard ratios (HR)
   - Confidence intervals (95% CI)
   - P-values
   - Sample sizes
   - Percentages
5. Look for data in tables, figures, and appendices
6. Primary endpoint data is usually in the main results section
7. Safety data is often in separate tables or appendices
...
- Ensure ci_lower_95 < hazard_ratio < ci_upper_95
- P-values are between 0 and 1 (use scientific notation if needed, e.g., 0.00001)

Return the extracted data as a JSON object matching the schema exactly.`;

    // Generate content with PDF files
    ...
    console.log('🤖 Gemini is analyzing the PDF(s)...');
    ...
    // Clean up: delete uploaded files from Gemini
    console.log('🧹 Cleaning up uploaded files...');
    for (const file of uploadedFiles) {
      try {
        await fileManager.deleteFile(file.name);
      } catch (deleteError) {
        console.warn(`Warning: Could not delete file ${file.name}:`, deleteError.message);
      }
    }
    ...
    const warnings = identifyDataWarnings(extractedData);
    ...
    return {
      ...
      file_names: uploadedFiles.map(f => f.displayName)
    };
 ...
 */
async function waitForFilesActive(files, maxWaitTime = 120000) {
  ...
  for (const file of files) {
    while (true) {
      ...
      if (fileStatus.state === 'ACTIVE') {
        console.log(`✓ ${fileStatus.displayName} is ready`);
        break;
      }

      if (fileStatus.state === 'FAILED') {
        throw new Error(`File processing failed: ${fileStatus.displayName}`);
      }

      if (Date.now() - startTime > maxWaitTime) {
        throw new Error(`Timeout waiting for file processing: ${fileStatus.displayName}`);
      }
...
function generateExtractionSummary(data, identifier) {
  ...
  if (data.efficacy_outcomes?.primary_endpoint_data?.hazard_ratio) {
    ...
    const ci_lower = data.efficacy_outcomes.primary_endpoint_data.ci_lower_95;
    const ci_upper = data.efficacy_outcomes.primary_endpoint_data.ci_upper_95;
    summary.en += `Hazard ratio: ${hr.toFixed(2)}`;
    if (ci_lower && ci_upper) {
      summary.en += ` (95% CI: ${ci_lower.toFixed(2)}-${ci_upper.toFixed(2)})`;
    }
    summary.en += '. ';
  }
  ...
  if (data.efficacy_outcomes?.primary_endpoint_data?.hazard_ratio) {
    ...
    const ci_lower = data.efficacy_outcomes.primary_endpoint_data.ci_lower_95;
    const ci_upper = data.efficacy_outcomes.primary_endpoint_data.ci_upper_95;
    summary.es += `Hazard ratio: ${hr.toFixed(2)}`;
    if (ci_lower && ci_upper) {
      summary.es += ` (IC 95%: ${ci_lower.toFixed(2)}-${ci_upper.toFixed(2)})`;
    }
    summary.es += '. ';
  }

...
import { FileJson, Play, AlertCircle, CheckCircle2, Wand2, Sparkles } from 'lucide-react';
import LoadingSpinner from './LoadingSpinner';
import TrialDataExtractor from './TrialDataExtractor';
import { processGemJSON } from '../utils/jsonCleaner';
import { t } from '../i18n/translations';

export default function DataInput({ onAnalyze, language }) {
  ...
  const handleAIExtractedDataA = (data) => {
    setTrialAJson(JSON.stringify(data, null, 2));
    validateJSON(JSON.stringify(data, null, 2), setValidationA);
  };

  const handleAIExtractedDataB = (data) => {
    setTrialBJson(JSON.stringify(data, null, 2));
    validateJSON(JSON.stringify(data, null, 2), setValidationB);
  };
    ...
    <div className="animate-fade-in">
      ...
      <div className="card p-6 mb-8">
        <div className="flex items-start space-x-3">
          ...
          <button
            onClick={() => setUseAIExtractor(!useAIExtractor)}
          ...
          >
            ...
            <span className="font-medium">{useAIExtractor ? 'Manual Mode' : 'AI Extractor'}</span>
          </button>
        </div>
      </div>
      ...
      {useAIExtractor && (
        <div className="space-y-6 mb-8">
          <div className="card p-6">
            <TrialDataExtractor
              onDataExtracted={handleAIExtractedDataA}
              trialIdentifier="A"
              language={language}
            />
          </div>
          ...
          <div className="card p-6">
            <TrialDataExtractor
              onDataExtracted={handleAIExtractedDataB}
              trialIdentifier="B"
              language={language}
            />
          </div>
      ...
      {/* Input Cards - Only show in manual mode */}
      {!useAIExtractor && (
